...
class AnimatedTerminalBar {
  ...
  private getColor(colorIndex: number, colorRange: number, startColor: number[]) {
    const color = startColor.map((c, i) => {
      ...
      const colorValue = (c + colorIndex + i * 2) % 256;
  ...
  private getBar(colorIndex: number, colorRange: number, startColor: number[]) {
    ...
    for (let i = 0; i < AnimatedTerminalBar.BAR_LENGTH; i++) {
      const color = this.getColor(colorIndex + i, colorRange, startColor);
      coloredBar += this.colorText(AnimatedTerminalBar.BAR_CHAR, i + this.spinner.t);
    }
    this.spinner.t = (this.spinner.t + 1) % AnimatedTerminalBar.BAR_LENGTH;
    return coloredBar;
  }
  public start() {
    let colorIndex = 0;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const codeUpdateAgent = new CodeUpdateAgent(projectRoot, []);
    ...
    function summaryInfo() {
        ...
        if (working) {
          ...
          if(line === 'clear') {
            ...
            return;
          ...
          else if(line.startsWith('addopen')) {
              const openFiles = vscode.window.visibleTextEditors.map(e => e.document.fileName);
              for(const fileName of openFiles) {
              ...
              writeEmitter.fire(`'addopen' - add all open files to the project\r\n`);
